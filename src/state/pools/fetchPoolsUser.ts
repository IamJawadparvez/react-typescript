...
import {getMasterchefContract, getSouschefContract, getSmartChefWBNBContract} from 'utils/contractHelpers'
...
const smartChefWBNBContract = getSmartChefWBNBContract()
...
export const fetchUserPendingBNBRewards = async (account) => {
  const calls = nonMasterPools.map((p) => ({
    address: getAddress(p.contractAddress),
    name: 'pendingReward',
    params: [account],
  ...
  const res = await multicall(smartChef, calls)
  const pendingBNBRewards = nonMasterPools.reduce(
    (acc, pool, index) => ({
      ...acc,
      [pool.sousId]: new BigNumber(res[index]).toJSON(),
    }),
  ...
  // Apple / Apple pool
  const pendingBNBReward = await smartChefWBNBContract.getUnpaidEarnings(account)

  return { ...pendingBNBRewards, 888: new BigNumber(pendingBNBReward.toString()).toJSON() }
}
